...
@property (nonatomic, copy) NSDictionary<NSString *, SVGAImage *> *images;
...
- (void)resetImagesWithJSONObject:(NSDictionary *)JSONObject {
    if ([JSONObject isKindOfClass:[NSDictionary class]]) {
        NSMutableDictionary<NSString *, SVGAImage *> *images = [[NSMutableDictionary alloc] init];
...
//                NSData *imageData = [NSData dataWithContentsOfFile:filePath];
            ...
            if ([SVGAVideoEntity isMP3Data:protoImages[key]]) {
                ...
                // ✅ 使用 SVGAImage 替代 UIImage（延迟解码）
                SVGAImage *image = [[SVGAImage alloc] initWithData:protoImages[key] scale:2.0];
                if (image != nil) {
                    [images setObject:image forKey:key];
                }
            }
